...
ROS2D.OccupancyGridSrvClient = function(options) {
  ...
  options = options || {};
  var ros = options.ros;
  var service = options.service || '/map_server/map';
  this.rootObject = options.rootObject || new createjs.Container();
...
ROS2D.PathShape.prototype.setPath = function(path) {
	...
	if (path !== null && typeof path !== 'undefined') {
		...
		this.graphics.beginStroke(this.strokeColor);
		//this.graphics.moveTo(path.poses[0].pose.position.x / this.scaleX, path.poses[0].pose.position.y / -this.scaleY);
    if ( path.poses.length = 2)
      this.graphics.moveTo(path.poses[0].pose.position.x / path.poses[0].pose.position.y / -this.scaleY).lt(path.poses[1].pose.position.x / this.scaleX, path.poses[1].pose.position.y / -this.scaleY).lt(path.poses[1].pose.position.x / this.scaleX, path.poses[1].pose.position.y / -this.scaleY);
    else if ( path.poses.length = 3)
    ...
    else if ( path.poses.length >= 4  && path.poses.length < 6)
    ...
    else if ( path.poses.length >= 6  && path.poses.length < 9)  
    ...
    else if ( path.poses.length >= 9  && path.poses.length < 12)  
...
ROS2D.TraceShape.prototype.addPose = function(pose) {
	...
	if (last < 0) {
		...
		var prevX = this.poses[last].pose.position.x;  // add pose
		var prevY = this.poses[last].pose.position.y;  // add pose
